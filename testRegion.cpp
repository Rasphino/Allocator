...
public:
    vecWrapperT(int type, std::vector<T, MyLib::RegionAllocator<T>> *pVec) {
        m_type = type;
        m_pVec = pVec;
    }

    virtual ~vecWrapperT() {
        if (m_pVec) delete ((std::vector<T, MyLib::RegionAllocator<T>> *) m_pVec);
    }

public:
    virtual void
    visit(int index) { T temp = (*(std::vector<T, MyLib::RegionAllocator<T>> *) m_pVec)[index]; }

    virtual int size() { return ((std::vector<T, MyLib::RegionAllocator<T>> *) m_pVec)->size(); }

    virtual void resize(int size) {
        ((std::vector<T, MyLib::RegionAllocator<T>> *) m_pVec)->resize(size);
    }

    virtual bool checkElement(int index, void *pValue) {
        T temp = (*(std::vector<T, MyLib::RegionAllocator<T>> *) m_pVec)[index];
        if (temp == (*((T *) pValue)))
            return true;
        else
            return false;
    ...
    virtual void setElement(int index, void *value) {
        (*(std::vector<T, MyLib::RegionAllocator<T>> *) m_pVec)[index] = *((T *) value);
    }
};
...
int main() {
    ...
    for (int i = 0; i < TESTSIZE - 4; i++) {
        ...
        vecWrapperT<int> *pNewVec = new vecWrapperT<int>(INT,
                                                         new std::vector<int, MyLib::RegionAllocator<int>>(
                                                                 tSize));
    ...
    for (int i = 0; i < 4; i++) {
        ...
        vecWrapperT<myObject> *pNewVec =
                new vecWrapperT<myObject>(CLASS,
                                          new std::vector<myObject, MyLib::RegionAllocator<myObject>>(
                                                  tSize));
